import { twoline2satrec, propagate, gstime, eciToGeodetic } from "../../libs/satellite.js/dist/satellite.es.js";
    ...
    if (changedStates["clickedSatellite"]) {
        ...
        if (clickedSatellite) {
            ...
            if (tleData && tleData.first && tleData.second) {
                console.log("TLE Data for clicked satellite:", tleData);
                ...
                if (simulation) {
                    ...
                    addSeparatorLine(graphContainer);
                    renderGraph("Height (km)", simulation.labels, simulation.altitudeData, simulation.times, graphContainer);
                    addSeparatorLine(graphContainer);
                    renderGraph("Latitude (°)", simulation.labels, simulation.latitudeData, simulation.times, graphContainer);
                    addSeparatorLine(graphContainer);
                    renderGraph("Longitude (°)", simulation.labels, simulation.longitudeData, simulation.times, graphContainer);
                }
            } else {
                console.log("No valid TLE data found for clicked satellite:", clickedSatellite);
                clearGraphs();
                showNoSatelliteMessage();
            }
        } else {
