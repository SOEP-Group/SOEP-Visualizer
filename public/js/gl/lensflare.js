...
class Flare extends THREE.Object3D {
  ...
  project(camera) {
    this.lensPosition = this.position
      .clone()
      .project(camera)
      .multiplyScalar(0.5)
      .addScalar(0.5);
...
class LensFlarePass extends Pass {
  constructor(scene, camera, flares, { coverageScale = 1 } = {}) {
    super();
    this.width = 1;
    this.height = 1;
    this.scene = scene;
    this.camera = camera;
    this.flareRenderTarget = new THREE.WebGLRenderTarget(
      this.width / 2,
      this.height / 2,
      {
    ...
    this.transparencyRenderTargetDWTrue = new THREE.WebGLRenderTarget(
      this.width,
      this.height,
      {
        minFilter: THREE.LinearFilter,
    ...
    this.transparencyRenderTargetDWTrue.depthTexture = new THREE.DepthTexture(
      this.width,
      this.height,
      THREE.UnsignedIntType
    );
    this.transparencyRenderTargetDWFalse = new THREE.WebGLRenderTarget(
      this.width,
      this.height,
      {
        minFilter: THREE.LinearFilter,
    ...
    this.transparencyRenderTargetDWFalse.depthTexture = new THREE.DepthTexture(
      this.width,
      this.height,
      THREE.UnsignedIntType
    );
    this.depthCopyPass = new FullScreenQuad(
    ...
    this.outputQuad = new FullScreenQuad(
      new THREE.ShaderMaterial({
        uniforms: {
          tDiffuse: { value: null },
...
vec4 texture_bicubic(sampler2D tex, vec2 uv, vec4 texelSize)
{
    ...
    const flareTransparentBlockers = new THREE.InstancedMesh(
      new THREE.CircleGeometry(1.25 * coverageScale, 32),
    ...
    this.coverageQuad = new FullScreenQuad(
      new THREE.ShaderMaterial({
        uniforms: {
          ...
          transparencyDWFalse: {
            value: this.transparencyRenderTargetDWFalse.texture,
          },
          transparencyDWTrue: {
            value: this.transparencyRenderTargetDWTrue.texture,
          },
          transparencyDWTrueDepth: {
            value: this.transparencyRenderTargetDWTrue.depthTexture,
          },
    ...
    this.flareQuad = new FullScreenQuad(
      new THREE.ShaderMaterial({
        uniforms: {
          flare: {
            value: Array(4),
...
c.r+=f1+f2+f4+f5+f6; c.g+=f1+f22+f42+f52+f62; c.b+=f1+f23+f43+f53+f63;
  ...
  renderTransparency(renderer) {
    ...
    for (let i = 0; i < this.flares.length; i++) {
      ...
      if (
        this.flares[i].enabled &&
        this.flares[i].visible &&
        this.flares[i].opacity > 0
      ) {
  ...
  render(renderer, inputBuffer, outputBuffer) {
    ...
    const flaresToRender = this.flares.filter(
      (flare) => flare.enabled && flare.visible && flare.opacity > 0
    );
    for (let i = 0; i < flaresToRender.length; i += 4) {
      ...
      renderer.setRenderTarget(this.coverageTarget);
      renderer.clear();
      this.coverageQuad.material.uniforms.depthTexture.value =
        this.depthTexture;
      batch.forEach((flare, index) => {
        this.coverageQuad.material.uniforms.lensPosition.value[index] =
          flare.position;
      });
      if (batch.length < 4) {
        // Pad with dummy values
        for (let j = batch.length; j < 4; j++) {
          this.coverageQuad.material.uniforms.lensPosition.value[j] =
            new THREE.Vector3(0, 0, 0);
        }
      }
      this.coverageQuad.material.uniforms.doTransparency.value =
        this.doTransparency;
      this.coverageQuad.material.uniforms.projMat.value.copy(
        this.camera.projectionMatrix
      );
      this.coverageQuad.material.uniforms.viewMat.value.copy(
      ...
      renderer.autoClear = false;
      this.flareQuad.material.uniforms.depthTexture.value = this.depthTexture;
      this.flareQuad.material.uniforms.coverageTexture.value =
        this.coverageTarget.texture;
      this.flareQuad.material.uniforms.time.value = performance.now() / 1000;
      this.flareQuad.material.uniforms.flareIndices.value = batch.map((flare) =>
        this.flares.indexOf(flare)
      );
      batch.forEach((flare, index) => {
        this.flareQuad.material.uniforms.flare.value[index] = flare;
        this.flareQuad.material.uniforms.lensPosition.value[index] =
          flare.lensPosition;
      });
      if (batch.length < 4) {
        // Pad with dummy values
        for (let j = batch.length; j < 4; j++) {
          ...
          this.flareQuad.material.uniforms.lensPosition.value[j] =
            new THREE.Vector3(0, 0, 0);
        }
      }
      this.flareQuad.material.uniforms.numFlares.value = batch.length;
      this.flareQuad.material.uniforms.resolution.value.set(
        this.flareRenderTarget.width,
        this.flareRenderTarget.height
      );
      this.flareQuad.material.uniforms.cameraPos.value.copy(
        this.camera.getWorldPosition(new THREE.Vector3())
      );
      this.flareQuad.material.uniforms.cameraDirection.value.copy(
    ...
    this.outputQuad.material.uniforms.texelSize.value.set(
      1 / this.flareRenderTarget.width,
      1 / this.flareRenderTarget.height,
      this.flareRenderTarget.width,
      this.flareRenderTarget.height
    );
